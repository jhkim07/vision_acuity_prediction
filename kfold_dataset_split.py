...
def main():
    ...
    # Define output paths based on the label
    OUTPUT_TEST_PATH = f"./data_split/{label}_test_data.json"         # 테스트 데이터 저장 경로
    OUTPUT_TRAIN_PATH = f"./data_split/{label}_train_data.json"       # 훈련 데이터 저장 경로
    OUTPUT_VAL_PATH = f"./data_split/{label}_validation_data.json"    # 검증 데이터 저장 경로
    OUTPUT_ALL_PATIENTS_PATH = f"./data_split/{label}_all_patients_split.json"  # 모든 환자 ID 저장 경로
    ...
    # 테스트 데이터 생성 (파일 이름과 레이블만 포함)
    test_data = [[file, label] for pid in selected_test_ids for file in patients[pid]]
    ...
    if n_splits > 1:
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kf.split(remaining_ids)):
            ...
            # Train 데이터 생성 (파일 이름과 레이블만 포함)
            train_data = [[file, label] for pid in train_patients for file in patients[pid]]

            # Validation 데이터 생성 (파일 이름과 레이블만 포함)
            val_data = [[file, label] for pid in val_patients for file in patients[pid]]
